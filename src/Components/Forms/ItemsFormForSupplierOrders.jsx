...
export const ItemsFormForOrders = ({
  getData = () => {},
  supplier = undefined,
  remove,
  register,
  ...
  useEffect(() => {
    ...
    if (selectedItem)
      dataFetch({
        simpleurl: "catalog/getcatalogbyitemsimple",
        init: formatInit({ data: { item_id: selectedItem.id } }),
      }).then((r) => setInfo(r));
  }, [selectedItem]);
    ...
    <Div>
      <StyledInput
        ...
        onChange={(e) => {
          ...
          setValue(
            `items.${index}.valor`,
            parseFloat(getValues(`items.${index}.valor`)).toFixed(2)
          );
          setValue(
            `items.${index}.total`,
            parseFloat(
              getValues(`items.${index}.quantidade`) *
                getValues(`items.${index}.valor`)
            ).toFixed(2)
          );
        }}
      />
      {supplier && (
        <input
          title="marque para selecionar somente produtos do catálogo"
          type="checkbox"
          onClick={() => {
            setChecked(!checked);
          }}
      ...
      <Select
            ...
            if (checked) {
              ...
              var newValue = `${
                typeof value.valor === "string"
                  ? parseFloat(Number(value.valor.replace(",", "."))).toFixed(2)
                  : parseFloat(value.valor).toFixed(2)
              }`;
              setValue(`items.${index}.valor`, newValue);
              setValue(
                `items.${index}.total`,
                parseFloat(getValues(`items.${index}.quantidade`) * newValue)
              );
            }
          } catch (error) {}
        }}
        url={
          checked
            ? `catalog/getcatalogitemsbysupplier/${supplier}`
            : "items/getitems"
        }
      />
      {showInfo && (
      ...
      <StyledInput
        ...
        onBlur={(e) => {
          ...
          setValue(
            `items.${index}.total`,
            parseFloat(
              getValues(`items.${index}.quantidade`) * e.target.value
            ).toFixed(2)
          );
        }}
        type="Number"
