...
export const TrowComponent = ({row = {}, rowNames = [], allowEdit = false, onSubmit = () => {}, crudUrls = {}, width = '100%', reset = () => {}, onClickInaRow = () => {}})=>{
    ...
    return <>
    {data &&
    <TRow onClick={()=>{ onClickInaRow(data)}}> 
    <Modal show={show} width={'100px'} onClose={()=>{setShow(false)}} children={info}/>
        {editing ?
            ...
            Object.values(data).map((each, index) => {
                ...
                var value = `${typeof each === 'string' ? each : Number.parseFloat(each).toFixed(2)}`;
                if( Object.keys(data)[index] === 'id' | Object.keys(data)[index] === 'quantidade'){
                    value = each
                }
                return <div className={classname} onClick={()=>{setShow(!show);setInfo(each)}} title={each} style={{position: 'relative'}}  key={index} >{value}</div>
            })
        }
        {allowEdit && <>
            {!editing ? 
                <div className="col-functions">
                    ...
                    <StyledCancelButton onClick={()=>{dataFetch({simpleurl: crudUrls.d, init: formatInit({data: data})}).then(r=>reset())}}>Excluir</StyledCancelButton>
                </div>
                : 
                <>
